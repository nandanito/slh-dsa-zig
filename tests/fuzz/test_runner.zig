//! Default test runner for unit tests — vendored with a one-spot patch so
//! that `--fuzz` compiles on Zig 0.16.0.
//!
//! Provenance
//! ----------
//! Verbatim copy of the stock runner shipped with Zig 0.16.0:
//!   lib/zig/compiler/test_runner.zig
//!   sha256 63adeb754894468560fee4cb0ac48af96f9fb71a00ccb5910e2ed8c153b56199
//! with exactly ONE change (marked `PATCH(slh-dsa-zig)` below).
//!
//! Why this file exists
//! --------------------
//! Zig 0.16.0's stock runner does not compile when built in fuzz mode
//! (`zig test -ffuzz`, i.e. `zig build … --fuzz`). In the crash-reporting
//! path it calls `std.debug.writeStackTrace(trace, …)` with `trace` from
//! `@errorReturnTrace()`, whose type is `*std.builtin.StackTrace`, but
//! `writeStackTrace` now takes `*const std.debug.StackTrace`. Those two
//! `StackTrace` types were split in 0.16.0 and the runner was not updated,
//! so the reference is a hard *compile* error reachable only under `-ffuzz`.
//! A custom test runner (`addTest(.{ .test_runner = … })`) is the supported
//! escape hatch: this vendored copy converts the builtin trace to a
//! `std.debug.StackTrace` before printing.
//!
//! Maintenance
//! -----------
//! This is coupled to Zig 0.16.0 internals. On any Zig upgrade, re-vendor
//! from the new stock runner and re-check whether the patch is still needed
//! (once upstream fixes the bug, delete this file and drop the
//! `.test_runner` wiring in build.zig — the harnesses use only the public
//! `std.testing.fuzz` API and need no other changes). Lane A only; it does
//! not touch upstream-candidate/. See issue #9.
        ...
        fn test_one() callconv(.c) bool {
            ...
            testOne(ctx, @constCast(&testing.Smith{ .in = null })) catch |err| switch (err) {
                error.SkipZigTest => return true,
                    ...
                    p: {
                        if (@errorReturnTrace()) |trace| {
                            // PATCH(slh-dsa-zig): `trace` is a `*std.builtin.StackTrace`,
                            // but `writeStackTrace` takes a `*const std.debug.StackTrace`.
                            // These types were split in Zig 0.16.0; the stock runner was
                            // not updated, so the original line is a compile error under
                            // `-ffuzz`. Convert before printing. See the file header.
                            ...
                            std.debug.writeStackTrace(&dt, stderr) catch break :p;
                        }
